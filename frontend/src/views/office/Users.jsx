...
export default function Users() {
  ...
  async function handleCreateFE() {
    ...
    try {
      await createFE({
        displayName: feDisplayName.trim(),
        email: feEmail.trim(),
      }).unwrap();
      setShowCreateFE(false);
      setFeDisplayName(""); setFeEmail("");
      setMsg("Field Engineer created. Credentials will be emailed automatically.");
      refetchFE();
    } catch (e) {
  ...
  async function handleCreateCustomer() {
    ...
    try {
      ...
      setMsg(custCreatePortal
        ? "Customer + Portal created; credentials emailed."
        : "Customer created."
      );
      refetchCustomers();
    } catch (e) {
        ...
        <ModalShell onClose={() => setShowCreateFE(false)}>
          <DialogCard title="Create Field Engineer" variant="info">
            ...
            <div className="mt-4 flex items-center justify-end gap-3">
              ...
              <Button onClick={handleCreateFE} disabled={creatingFE} className="min-w-[140px]">
                {creatingFE ? "Creating…" : "Create Field Engineer"}
              </Button>
            </div>
          </DialogCard>
        </ModalShell>
        ...
        <ModalShell onClose={() => setShowCreateCust(false)}>
          <DialogCard title="Create Customer" variant="info">
            ...
            <div className="mt-4 flex items-center justify-end gap-3">
              ...
              <Button onClick={handleCreateCustomer} disabled={creatingCust} className="min-w-[120px]">
